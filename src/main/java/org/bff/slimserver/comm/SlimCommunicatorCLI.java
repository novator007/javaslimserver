 ...
 * @author bfindeisen
 */
public class SlimCommunicatorCLI implements SlimCommunicator {
    ...
    private static final String PREFIX_RESCAN = SlimConstants.RESPONSE_PREFIX_RESCAN;

    public SlimCommunicatorCLI(String server) throws UnknownHostException, SlimConnectionException {
        this(server, DEFAULT_CLI_PORT);
    }

    public SlimCommunicatorCLI(String server, int cliPort) throws UnknownHostException, SlimConnectionException {
        this(server, cliPort, null, null);
    }

    public SlimCommunicatorCLI(String server, int cliPort, String user, String password) throws UnknownHostException, SlimConnectionException {
    ...
    private synchronized void connect() throws SlimConnectionException {
        try {
            this.socket = new Socket(getServerAddress(), getServerPort());

            if (getUser() != null && getPassword() != null) {
                sendCommand(new SlimCommand(CMD_LOGIN,
                        new String[]{getUser(), getPassword()}));
            }

            //I do this twice to check the connection.  If the user and password are incorrect
            //this will bomb
            sendCommand(new SlimCommand(CMD_VERSION));

        } catch (Exception e) {
            throw new SlimConnectionException(e.getMessage());
        }
        if (!this.socket.isConnected()) {
            throw new SlimConnectionException();
        }
    }

    @Override
    public synchronized String[] sendCommand(String slimCommand) throws SlimConnectionException {
        return sendCommand(slimCommand, null);
    }

    @Override
    public synchronized String[] sendCommand(String slimCommand, String param) throws SlimConnectionException {
        if (param == null) {
            return sendCommand(new SlimCommand(slimCommand));
        } else {
            return sendCommand(new SlimCommand(slimCommand, param));
        }
    }

    @Override
    public synchronized String[] sendCommand(SlimCommand command) throws SlimConnectionException {
        ...
        try {
            ...
            Logger.getLogger(SlimServer.class.getName()).log(Level.SEVERE, null, e);
    ...
    private static void loadProperties() {
        ...
        try {
            prop.load(is);
            setEncoding(SlimServer.getEncoding());
            CMD_VERSION = prop.getProperty(SlimConstants.PROP_CMD_VERSION);
            CMD_LOGIN = prop.getProperty(SlimConstants.PROP_CMD_LOGIN);
        } catch (Exception e) {
            Logger.getLogger(SlimServer.class.getName()).log(Level.SEVERE, null, e);
        } finally {
            try {
                ...
                Logger.getLogger(SlimCommunicatorCLI.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    }
    ...
    private String loadVersion() throws SlimConnectionException {
        return sendCommand(new SlimCommand(CMD_VERSION))[0];
    }
