 ...
 * @author bfindeisen
 */
public class SlimFavoritePlugin implements SlimPlugin {
     ...
     */
    public SlimFavoritePlugin(SlimServer slimServer) {
    ...
    public boolean isFavorite(String trackIdorURL) throws SlimException {
    ...
    public SlimFavoriteAudioDetails getFavoriteAudioDetails(SlimFavorite favorite) throws SlimException {
        ...
        SlimFavoriteAudioDetails details = new SlimFavoriteAudioDetails(favorite.getId(), getCommand());
        ...
        for (int i = 0; i < response.length; i++) {
            ...
            value:db:contributor.namesearch=2%20LIVE%20CREW
            url:db:contributor.namesearch=2%20LIVE%20CREW
            type:audio
             */
            if (response[i].startsWith(PREFIX_NAME)) {
                ...
                try {
                    details.setIconUrl(new URL("http://" + getSlimServer().getServer() + ":" + getSlimServer().getWebPort() + "/" + response[i].replace(PREFIX_ICON, "")));
     ...
     */
    public void clearFavorites() throws SlimException {
        int count = getCount();
    ...
    public Collection<SlimFavoriteItem> getFavorites() throws SlimException {
        return getFavorites((String) null);
    }

    public Collection<SlimFavoriteItem> getFavorites(SlimFavorite favorite) throws SlimException {
        return getFavorites(favorite.getId());
    }

    public void loadFavoriteItem(SlimFavoriteItem item) throws SlimException {
        logger.trace("Starting LoadFavoriteItem  for " + item.getName());

        for (SlimFavoriteItem sfi : getFavorites(item.getId())) {
            item.addFavoriteItem(sfi);
        }

    }

    public int getCount() throws SlimException {
    ...
    private Collection<SlimFavoriteItem> getFavorites(String id) throws SlimException {
        logger.trace("Starting GetFavorites");
        List<SlimFavoriteItem> favs = new ArrayList<SlimFavoriteItem>();
        String command = SS_PROP_FAVORITE_ITEMS.replaceAll(PARAM_START, "0");
        command = command.replaceAll(PARAM_ITEMS, Long.toString(MAX_SEARCH_RESULTS));
        if (id == null) {
            command = command.replaceAll(PARAM_TAGS, "");
...
//        }
        ...
        for (int i = 0; i < response.length; i++) {
            ...
            if (response[i].startsWith(PREFIX_ID)) {
                ...
                SlimFavoriteItem fav = new SlimFavoriteItem(str);
                ...
                if (fav.isContainsItems() && !fav.isAudio()) {
                    fav.setSlimType(SlimFavoriteItem.SLIMTYPE.FOLDER);
                }
    ...
    public boolean addFolder(String folder) throws SlimException {
    ...
    public void addFavoriteToFolder(SlimPlayableObject item, SlimFavoriteItem folder) throws SlimException {
    ...
    public void addFavorite(String title, String url) throws SlimException {
    ...
    public void addFavorite(SlimPlayableObject item) throws SlimException {
        addFavorite(item.getTitle(), item.getUrl());
    }

    public void deleteFavorite(SlimFavorite favorite) throws SlimException {
    ...
    public void renameFavorite(SlimFavorite favorite, String newName) throws SlimException {
    ...
    public void moveFavorite(SlimFavorite fromFavorite, SlimFavorite toFavorite) throws
            SlimException {
